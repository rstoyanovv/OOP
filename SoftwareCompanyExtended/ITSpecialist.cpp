...
ITSpecialist::ITSpecialist(std::string name, const unsigned int experience, const double salary) : Employer(name, experience, salary)
{
	this->capacity = 10;
	this->size = 0;
...
ITSpecialist::ITSpecialist(const ITSpecialist& other) : Employer(other)
{
	this->copy(other);
}

ITSpecialist::ITSpecialist(ITSpecialist&& other) : Employer(std::move(other))
{
